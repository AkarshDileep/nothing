...
function App() {
  ...
  return (
    <div className="relative w-full h-[800vh]">
      ...
      <div id="projects" className="absolute top-[3600px] left-0 w-full h-px pointer-events-none"></div>
      ...
      <div className="sticky top-0 w-full h-screen z-20 flex items-end justify-center pointer-events-none overflow-hidden" style={{ perspective: '1500px' }}>
        ...
        <div
          ...
          style={{
            opacity: phase5Progress,
            transform: `translateZ(${(1 - phase5Progress) * 500}px) translateY(${(1 - phase5Progress) * 100}px)`,
            visibility: phase5Progress > 0 ? 'visible' : 'hidden',
          }}
        >
          <div className="w-full max-w-7xl mx-auto h-full flex flex-col justify-center">
            <h2 className="text-3xl md:text-5xl font-bold font-typewriter text-white mb-8 md:mb-12 text-center drop-shadow-lg tracking-wider uppercase">
              Selected Works
            </h2>

            <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 custom-scrollbar pr-2 pb-12 max-h-[80vh] md:max-h-[70vh] ${phase5Progress === 1 ? 'overflow-y-auto overflow-x-hidden' : 'overflow-hidden'}`}>
